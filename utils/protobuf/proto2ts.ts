...
class TypeLayers {
  ...
  dump(type: "ts", config: {
    ...
    switch (type) {
      case "ts":
        const in_path = config.src_path.split(/[/\\]/);
        let file_name = in_path.pop();
        file_name = config.out_name ?? file_name;
        const out_file = (config.dest_dir ?? in_path.join("/")) + "/" + file_name;
 ...
 */\n\n`;
        const [ts, dts, enums] = this.dump_ts(0, config.padding ?? 2);
        const enum_header = `import {\n${enums}\n} from "./${file_name}.ts";\n\n`;
        writeFileSync(out_file + ".ts", header + ts);
        writeFileSync(out_file + ".d.ts", header + enum_header + dts);
        break;
    }
  }
  /**
   ...
   * [Nodes]
   */
  private dump_ts(depth: number, padding: number): [ts: string, dts: string, enums: string] {
    ...
    if (depth > 0) {
      dts += "declare interface " + this.getPath().join("$") + " {\n";
    }
    for (const v of this.vars.values()) {
      // /** var_comments */
      // var_name?: var_type[]; // var_index
      if (v.comment) {
        dts += `${depth > 0 ? PAD : ""}/** ${v.comment} */\n`;
      }
      ...
      dts += `${depth > 0 ? PAD : ""}${var_name}: ${var_type}; // ${v.index}\n`;
    }
    if (depth > 0) {
      dts += "}\n";
    }
    ...
    for (const v of this.message.values()) {
      if (this.enums.has(v.name)) {
        ...
        ts += "} as const;\n";
        enums += `${PAD}${class_name},\n`;
        dts += `declare type ${class_name} = (typeof ${class_name})[keyof typeof ${class_name}];\n`;
      } else {
        const [t, d, e] = v.dump_ts(depth + 1, padding);
        ts += t;
        dts += d;
        enums += e;
      }
