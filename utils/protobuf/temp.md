...
// 区分系统来源
  ...
  int32                    is_explicitly_set = 2; // 0=空值, 1=用户已修改
  optional TypeDefinition  type_def          = 4; // 类型元数据
  ...
  oneof storage {
    ...
    Int              val_int    = 102; // 整数
    Float            val_float  = 104; // 浮点
    String           val_string = 105; // 字符串
    Enum             val_enum   = 106; // 枚举索引
    Vector           val_vector = 107; // 向量
...
message Int { int32 x = 1; }

message Float { float x = 1; }

message String { string str = 1; }

message Enum { int64 enum = 1; }
...
message StructStorage { repeated TypedValue fields = 1; }

message ListStorage { repeated TypedValue elements = 1; }
  ...
  message Config {
    message Inner {
      ...
      oneof schema_binding {
        TypedValue.TypeDefinition.StructReference    target_struct_id    = 100;
        TypedValue.TypeDefinition.StructReference    list_item_struct_id = 101;
        TypedValue.TypeDefinition.MapKeyValueBinding map_config          = 102;
      }
    }
    Inner inner = 1;
  }
...
message NodeInstance {
  ...
  // 结构体依赖声明
  // TODO: ???
  repeated StructInfo using_structs = 10;
}
...
message PinInstance {
  ...
  // 值与类型
  TypedValue value = 3;
  int32      type  = 4; // Be careful! ServerTypeId in Server and ClientVarType in Client
