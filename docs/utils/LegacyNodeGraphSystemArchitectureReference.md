...
#### 2.2.2 双重 ID 语义：GUID vs ID

在 `ResourceLocator` 中，存在两个 ID 字段，它们的含义截然不同：

| 字段名 (Refined) | 原始字段 | 语义 | 适用对象 | 示例 |
| --- | --- | --- | --- | --- |
| **Asset GUID** | `asset_guid` (4) | **资产库 GUID** | **资源管理层 / 文件系统**指向一个具体的、可序列化的用户资产文件。 | AssetBundle 依赖列表、Entity/Prefab 引用、系统内置节点(SYSTEM_DEFINED)通常没有此字段。 |
| **Runtime GUID** | `runtime_id` (5) | **运行时 ID** | **逻辑执行层 / 内存**指向一个逻辑定义、OpCode 或内存中的对象句柄。 | 节点图 ID、结构体定义 ID、信号 ID。 |
